...
public class MovieDao {
	...
	public MovieAvailablePeriod getMovieAvailableTime(String branchid, String movieid){
		try {
			String query = "SELECT m.startDate, m.endDate FROM movieavailable m WHERE m.branchid = ? AND m.movieid = ?";
	...
	public AvailableMovie.resultList getAvailableMovieByBranch(String branchid, String date) {
		...
		try {
			StringBuffer query = new StringBuffer().append("SELECT m.movieName,m.seqid,m.picURL, a.startDate, a.endDate FROM movie m, movieavailable a")
