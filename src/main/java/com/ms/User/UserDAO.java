...
public class UserDAO {
	...
	public List<UserGroupForm.Result> retrieveUserGroupList(){
		...
		try { 
			StringBuffer query = new StringBuffer().append("SELECT groupid, groupname FROM user_group WHERE groupid != ?");
	...
	public List<BranchForm.Result> retrieveBranchList(){
		...
		try { 
			StringBuffer query = new StringBuffer().append("SELECT b.seqid, b.branchName, d.districtname, s.stateName FROM branch b, district d, state s WHERE b.districtid = d.seqid ")
	...
	public List<UserModelList.Result> retrieveUsersDetails(String myUsername) {
		...
		try {
			StringBuffer query = new StringBuffer().append("SELECT s.seqid, s.username, u.groupname, s.status, b.branchName, s.createddate ")
												   .append("FROM staff s LEFT JOIN user_group u ON s.usergroup = u.groupid ")
												   .append("LEFT JOIN branch b ON s.branchid = b.seqid WHERE s.username != ? AND s.usergroup != ?");
	...
	public UserModel.Result retrieveUserDetails(String userid) {
		UserModel.Result result = null;
		try {
			StringBuffer query = new StringBuffer().append("SELECT s.seqid, s.username, u.groupname, s.status, b.branchName, s.createddate ")
												   .append("FROM staff s LEFT JOIN user_group u ON s.usergroup = u.groupid ")
												   .append("LEFT JOIN branch b ON s.branchid = b.seqid ")
			...
			if(records.size() > 0) {
				for(Map<String,Object> record : records) {
					...
					String branchname = Util.trimString((String)record.get("branchName"));
					String createddate = Constant.SQL_DATE_FORMAT.format((Timestamp)record.get("createddate"));
				    result = new UserModel.Result(seqid, username, Util.underscoreRemoval(Util.capitalize(groupname)), Util.replaceWithDash(Util.trimString(branchname)), Util.checkActivation(status), Constant.STANDARD_PLUGIN_WITHOUT_TIME.format(Constant.SQL_DATE_FORMAT.parse(createddate)));
				}
			}
		}
		catch(Exception ex) {
	...
	public boolean checkRedundantUsername(String username){
		...
		try {
			...
			StringBuffer query = new StringBuffer().append("SELECT seqid from staff where username = ?");
	...
	public Map<String,String> getEditInfo(String userid){
		...
		try {
			StringBuffer query = new StringBuffer().append("SELECT s.seqid, s.usergroup, s.branchid, s.username FROM staff s WHERE s.seqid = ? ");
			List<Map<String,Object>> records = jdbc.queryForList(query.toString(),userid);
			if(records.size() > 0) {
				for(Map<String,Object> record:records) {
	...
	public boolean updateUserStatus(String userid, int status) {
		try {
			StringBuffer query = new StringBuffer().append("UPDATE Staff SET status = ? WHERE seqid = ?");
	...
	public boolean updateUser(UserEditForm form) {
		try {
			StringBuffer query = new StringBuffer().append("UPDATE STAFF SET branchid = ?, usergroup = ? WHERE seqid = ?");
	...
	public boolean deleteUser(String userid) {
		...
		try {
			StringBuffer query = new StringBuffer().append("DELETE FROM STAFF WHERE seqid = ?");
	...
	public boolean addNewUser(NewUserForm form, String uuid, String createddate) {
		...
		try {
			...
			StringBuffer query = new StringBuffer("INSERT INTO STAFF VALUES(?,?,?,?,?,?,?)");
